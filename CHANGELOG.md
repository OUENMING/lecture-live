...
> 下面这几行**就是更新后那张卡片上显示的内容**（每行一条，按影响排序）。
> 卡片**优先读本节**，读不到才退回去抓 `### Added` / `### Fixed` —— 那些是写给
> 维护者的，含文件名与符号名，不适合直接上卡片。
>
> 写法：分类标签沿用 Keep a Changelog 的 `Added` / `Changed` / `Fixed`（行业标准，
> 不译为中文）；**破坏性变更排最前**；一条一事；带上实测数字。
> ⚠️ 本项目还在测试阶段、读者是开发者，所以**刻意写得专业**（含符号名与实测值），
> 不做"面向终端用户的大白话改写"。将来若面向非开发者分发，再改这一节即可。

- **Added** 更新说明卡片 `whatsnew.py` —— 非模态毛玻璃，每版本一次，可勾「不再提示」
- **Added** 卡片内容改由本文件的「更新看点」驱动 —— 不再抓实现细节上卡片
- **Changed** 面板 scrim `0.45` → `0.38` —— 透射率 0.306 → 0.345，白底对比度 2.60 → 2.33:1
- **Fixed** `whatsnew` 与 `overlay` 的 ObjC 类名冲突 —— 卡片曾**静默**返回 None
- **Fixed** 卡片定位越界被 AppKit 夹回，覆盖字幕面板
