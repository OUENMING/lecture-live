...
- [ ] **嘈杂环境 / 远场收音** —— 见下方「为什么不做那些人声处理方案」
      ⚠️ **去混响（WPE）待重测**：曾一度测出"碎片 −31%"并当成唯一正收益，**已撤回**。
      错了两层：① `nara_wpe.wpe()` 要的是**复数 STFT**，我传的是时域波形
      → 形状静默通过但实测是**恒等变换**（RMS 1.00079→1.00079、相关 1.0000），
      根本没去混响；② 就算调用对了，该指标在 n=11 段时的**噪声底线是 ±18 个百分点**
      （见下），而那个"改善"只有 8pp。
      **正确的流式路径是 `OnlineWPE`**（`channel=1` 必须显式传、`taps=40` 太贵要用 10），
      **离线 `wpe_v8` 在本机是坏的**（5s 音频 3.7GB、输出 −79 dBFS）。**收益仍未知。**
