...
- **按需 AI 讲解 + 追问** — 没听懂就按「讲一下」，或直接在悬浮窗底部的输入框里问一句，AI 就着**这节课到目前为止的转录**当场讲解。**英文为主、中文只作 `中：` 点睛**（不整段翻译）；补充的外部背景另起 `背景：` 一行，与课堂内容明确分开；转录里没有依据时它会直说没有，不编。答案是流式的，读的时候**接管字幕区**——字幕在后台继续累积，退出后一条不少地回来。**追问不用再点输入框**，回车后接着打即可。课后你问过的每一条自动变成笔记里的 `问题::答案` 复习项。
- **聊天式悬浮窗** — 不是只能看最近三句。可展开到 60% 屏高**回看整节课**，翻上去看历史不会被新内容拽下来
- **远场收音** — 讲台离你 10 米也能收。靠滑动峰值归一化补电平（实测可用动态范围扩大 ~24dB），不靠降 VAD 阈值
- **课后自动归档** — 逐句实时落盘（`kill -9` 也只丢最后一句；✕ / Ctrl+C 停止时在途的半句也会冲刷落盘，不丢），结束后生成**双层 Obsidian 笔记**：复习层（**英文知识点详解** + 英文自测 + 本课术语表 + 你标星的重点，中文只做辅助）+ 默认折叠的**逐字完整转录**。**不设课程代码也照常归档**。
- **翻译随时开关** — 面板上点一下就在「双语」和「只英文」之间切；关掉中文时**仍会做英文矫正**，不会退回原始 ASR 错听
- **零成本兜底 + 自动恢复** — 云端翻译失败 / 断网时自动降级到本地 Qwen3-1.7B，课上不会突然哑掉；降级后每 90s 自动探测，云端恢复即切回（菜单栏 🎧 变 ⚠️ 提示降级中）
...
> **不设课程代码也照常归档**：`cl` 不带 `.course` 时，笔记仍会写进 Obsidian、会话文件仍进 `sessions/`，
...
│   ├── ❓ Review 复习自测       ← 英文问答，`问题::答案`，可被 Spaced Repetition 插件识别
│   ├── 💡 Terminology 术语表    ← 带首次出现时间戳，本地查表（零网络）
│   └── ⭐ 我标记的重点          ← 课上按过 ⭐ 的句子（原文 + EN）
└── 📜 完整逐句转录（默认折叠） ← **逐字保留**：EN / ZH / 原始 ASR，一句不删
```

**长课分块生成**：一堂 40 分钟的课有数百句，复习层一次性生成会撑爆输出上限而**整份丢失**；因此按 ~70 句分块生成知识点再合并，块间用 `[HH:MM]` 时间前缀标注进度，最后用各块小标题重写一份**全课**概览。
...
要绝对离线：`cl local`，或干脆不配 key —— 云端失败会自动降级到本地 Qwen3-1.7B（降级是暂时的：每 90s 探测，云端恢复自动切回）。代价是译文质量从 ⭐⭐⭐⭐⭐ 掉到 ⭐⭐。
...
- [x] 翻译关掉仍做上下文英文矫正
- [x] 按需 AI 讲解 + 追问线程（答案流式接管字幕区 · 英文为主中文点睛 · 问过的问题自动进复习项）
