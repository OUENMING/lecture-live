...
| **输出语言** | **中文为主 · 英文辅助**（中文讲解 + 关键处一行 `EN：` 保留教授的原文措辞/术语）。⚠️ **2026-09-18 由「英文为主」翻转而来**，理由见下 |
| **讲解长度** | **稍长可接受** —— 以"当场看懂"为准，**不设硬性短上限** |
| **翻译开关** | 现有「译 开/译 关」**保留不变**；讲解是**独立入口**，不被它替代、也不依赖它 |

### ⚠️ 输出语言的方向翻转（2026-09-18，作者实测后改）

**原设计是「英文为主 + 中文点睛」**，理由是调研里最强的那条反对意见 —— 小红书原话
*"千万不要开同步翻译，那变成阅读了"*，中文字幕把「听课」变成「读屏」。当时的判断是：
用英文主体让人留在听讲的语言里，从而绕开它。

**实测后翻转成「中文为主 + 英文辅助」**，作者原话：**"要不看不懂"**。

理由决定性：**讲解存在的全部意义就是让人看懂**，而他的英语不是母语 —— 理解有
**词汇覆盖率阈值**（不到 90% 基本读不动，见记忆 `owen-l2-comprehension-constraint`）。
一份读不懂的英文讲解，无论"语言环境"多正确，价值都是零。

**英文没有被丢掉，而是换了位置**：讲清用中文，教授的**原文措辞/术语仍留一行 `EN：`**。
因为**考试是英文的** —— 那个词必须在考卷上认得出、对得上 PPT。

**教训**：「帮用户留在目标语言里」是好意，但**好意不能压过可读性**。设计意图再正确，
用户说读不懂就是读不懂。
...
## Phase 2 — 问答引擎 + 线程状态  ✅ **已实现并通过独立验证（2026-09-18，commit 58676fa）**

> 引擎已可用：输入框提交 → `answerq` → `answer_worker` → 流式回答。
> **但面板里还看不到**（答案只走终端 echo）—— 渲染是 Phase 3。
> 顺带修掉一个**既有 bug**：点 ✕ 退出恒定卡 15s，且会丢掉正在转写的那一句。
...
## Phase 3 — UI 接线  ✅ **已实现并通过独立验证（2026-09-18，commit 74aa0dd）**
...
调研实测：`HEIGHT = 362`（收起），转录区只占 210px；展开态 60% 屏高（900px 屏 → 总高 540，转录区 440px）。

**而一段真实答案是 2000/1500/3150 字符**（实测 3 轮 completion_tokens 499/375/786，英文约 4 字符/token）。按 13pt 折行算 ≈ **250/345/535px**，再加结构行（定位行 + `背景：` + 几条 `中：`）约 +60px。

**→ 典型答案就已经超过收起态的整个转录区，最长的那次连展开态也吃满。** 所以"塞进一个新槽位"根本不成立——**只能接管那个本来就最大的区域**。

接管的实现代价也最小：`overlay.py::_render()` 是**唯一**喂 `self._tv.set_content(...)` 的地方（约 :961-974）。按"答案是否活跃"分支，答案活跃时喂答案行而不是字幕行即可。**字幕不会丢**——`_history` 只由 `finalize()` 改，`_render` 从不碰它，所以字幕在后台继续累积，退出答案自动重现。

### 答案行复用字幕卡片的形状（关键设计）

转录区的行池是**固定行高**的（`transcript_view.py` 的命门：`row_h` 让"滚动偏移→行索引"变成 O(1) 除法）。所以答案**必须预先折行成固定行**才能进池子。

而行的结构是 `(big 英文行, sub 中文行)` —— **这正好就是「英文为主 + 中文点睛」的形状**：
- `big` = 答案的一行英文
- `sub` = 该行跟随的 `中：` 点睛（没有就留空）

→ **零新增排版机制**，且视觉上天生同族（答案读起来就是"更长的字幕卡片"）。

### 必须处理的四个坑（调研标出）

1. **`IDLE_S = 6.0` 的自动回底会把人拽走** —— 读长答案读到一半被拉回底部。接管期间必须抑制。
2. **`_pending_appends` 换内容时会数错** —— items 从 N 条字幕换成 M 条答案再换回来。`_follow` 为真时无害（有 guard），但会误置 `_new_since_leave`。
3. **绝不按 token 改面板尺寸** —— `_apply_mode` 会调 `NSScreen`、重设全部 subview frame、`setFrame_display_`（窗口 resize + 全重绘），还会让转录池按新高度重建。**只在进入接管（开一行）与 `answer_done`（量一次最终文本）时改高度。**
4. **`answer_delta` 必须非紧急** —— 只有接管开始 / `answer_done` / 退出才 `urgent=True`。照 `stream_zh` 的"累积 + 非紧急"模式。另：`_render` 现在无条件写 gloss/draft 字符串且无缓存，长答案每帧 `setStringValue_` 是 O(len)，要加缓存。

### 连续输入（作者追加要求）

**回车提交后，输入框保持焦点**，可以接着打下一问，不用再点一次。

现状是反的：`_submit_input()` 提交后调 `_release_focus()`（清 first responder + resign key），所以每一次追问都要重新点输入框。真机日志证实了这点——三次提交之间都有一次重新点击。

**⚠️ 但这与「键盘不自留」不变量直接冲突**，必须一起设计：

那条不变量的判据是 `is_key and not _is_editing()`。输入框一旦保持焦点，`_is_editing()` 恒为真 → 面板可以无限期保持 key → **点面板空白处之后，其他 app 的按键可能被吞**（这正是 Phase 1 那个被证伪又修好的 bug）。

**取舍与定案：**
- 提交后**保持焦点**（连续输入优先）。理由是提交动作本身就是"我正在跟这个框交互"的强信号；此刻让出焦点是错的。
- **Esc 仍然显式让出**（已有的 `_cancel_input`），并且是唯一的"我打完了"出口。
- **点其他 app 会自然夺走 key** —— 面板是 `NonactivatingPanel` 且从不激活 app，key 跟随活跃 app。
- ⚠️ **必须在真机上专门验**：连续问 3 轮后，切到浏览器/PPT 按方向键与空格是否正常。**这条自动化测不了。** 若被吞：退回"提交后让出焦点，但把焦点还给输入框需要一次点击"的旧行为，或者改成"答案流结束后再让出"。
- 实现上，`_submit_input` 改为清空文本 + **重新 `makeFirstResponder_(self._input)`**（而不是 `_release_focus`）；Esc 路径不变。


### 两个新按钮

- 顶栏插在 **index 2**（`_btn_latest` 之后、`_btn_expand` 之前）。⚠️ 调研纠正了计划：`_btn_latest` 隐藏时**仍占 63px 隐形空位**，插在 index 0 会把这个洞留在按钮组**内部**。
- 空间够：现有 5 个按钮 242px，可用 386px。
- **「讲一下」** → `answerq.put(常量问题)`；转录块由 `answer_worker`/`answer_user_content` 稍后附加，所以问题可以是常量。
- **「新话题」** → `start_new_topic()`（`main.py:402-408`，**目前是死代码**）+ Overlay 侧清 `_answer` / answer 高度后重排。
- 两个回调必须经 `_load_overlay` 与 `Overlay.__init__` **穿线**（当前只有 `on_quit/on_flag/on_translate/on_submit` 四个）。
- ⚠️ 按钮在**主线程**（`pump()` → `sendEvent_`）：只能 `queue.put` / 置标志 / 打印。不许联网、不许 sleep、不许长持 `qa_lock`。

### 其余边界

- **`answer_delta` / `answer_done` 已在 `drain()` 里经 `getattr` 调用**（main.py:689-695）→ **渲染不需要改 main.py**。省略不崩，只是静默不显示。
- **术语行与答案共存**（固定区变成 `gloss_h + answer_h` 累加），代码里没有互斥规则。想互斥就是新产品的规则——而且**写进 `_terms` 是计划点名的反模式**。
- **`set_translating(False)` 会清 `_draft_zh_val`/`_terms`/`_pinned_term`**（:927-931）→ 答案缓冲**必须显式排除**，否则违反"译关仍可用"。
- **命名冲突**：`self._pinned` 已被"固定区高度"占用，`_pinned_term` 是术语。答案高度要第三个名字。
- **收起/展开的语义**：术语行收起一行是因为它是**指针**；答案**本身就是内容**。作者的"自动展开"决定正好避开这个矛盾——答案一出现就展开到 60%，读完收回。
...
## Phase 4 — 落盘（只写「我问过什么」）  ✅ **已实现并通过独立验证（2026-09-18，commit e7db984）**

> 实际做法与本节原描述有两处偏离，理由写在 commit message 里：①**合并**进既有的
> `❓ Review 复习自测`（不另开一节 + 再写一遍 `::`，同一份内容写两遍只会变噪音）；
> ②`close()` 收到的 `qa` 是**取快照的函数**而不是早取好的 list —— 收尾里等你回答
> "是否保存"可以停几分钟，早取会把这段窗口里回来的那轮问答整条丢掉。
...
## Phase 5 — 验证与收尾  ✅ **已完成（2026-09-18）**

### 最终状态

五个阶段全部实现、独立验证、提交。`tests/test_audit_regressions.py` **35 项全绿**
（1 项因会话锁屏跳过：锁屏时任何窗口都无法成为 key window，那条断言无意义）。

**新增闸门 R6–R9**，全部做过**变异验证**（把修复种回去确认会变红 —— 空的测试比
没有测试更危险）：

| | 盯什么 |
|---|---|
| R6 | 悬浮窗必须能构造 + 细线必须真的有颜色（构造期异常曾被静默吞掉） |
| R7 | 答案接管不得吃掉字幕；连续输入不得把「键盘不自留」不变量弄丢 |
| R8 | 答案折行完整性；超长词被静默裁掉 / 空回答清空屏幕 / 追问时两轮答案粘连 |
| R9 | 复习钩子里必须是**用户原话**（分隔符吞前半段 / `::` 伪造卡片边界 / 截断超限） |

### ⚠️ 只有真人能验的（自动化覆盖不到）

1. **「保持焦点」之后会不会吞键** —— 连续问 3 轮，然后切到浏览器 / PPT 按方向键与
   空格。这关系到 Phase 3 的连续输入取舍（见上文）。**若被吞**：退回"提交后让出
   焦点"，或改成"答案流结束后再让出"。
2. **⌘C / ⌘V / ⌘A 在输入框里的真实行为** —— Edit 菜单的**构造**已实测通过，
   但快捷键路由需要真按键（Phase 1 起就挂着）。
3. **真实鼠标点两个新按钮 + 真机打字手感** —— 合成事件在锁屏会话里投递不了。
4. **Spaced Repetition 插件是否真的把 `- q::a　（中：…）` 收成卡片** —— 格式与既有
   LLM 项完全同形（沿用而非新造），但插件不在本机运行环境里，未实测。
5. **`probe_scroll.py` 在锁屏会话里是空跑的**（0 个滚轮事件）—— 需要解锁后复跑。
...
- ~~线程跨课持久化~~ → **已定：不做**，换课清空
- **回答是否朗读？** —— 未讨论
- **是否复用 `polish.py` 的二次精修来改善回答质量？** —— 未验证。
  ⚠️ 下面这句原来是"该文件**不在 git 里**（只存在于工作区）" —— **2026-09-24 核实为过期**：
  `polish.py` / `tests/` 都已经在 git 里（`git ls-tree -r HEAD` 可验），本条待办原来的
  "新 clone 拿不到"这个顾虑**不成立**。要验证的只剩"精修能不能真的改善回答质量"本身。
