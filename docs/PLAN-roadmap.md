...
ClassLive 是一个**只能用终端启动**的 macOS 本地实时英译中课堂字幕工具：
`cl` 是 bash 启动器 → `.venv/bin/python main.py` → 悬浮窗。

它已经能跑、能用、能更新，但有三件事没做：
**① 不能脱离终端** ② **没有自己的数据地盘**（一切依赖 Obsidian vault）③ **课件没有入口**。
...
| # | 阶段 | 做什么 | 依赖 | 成本 |
|---|---|---|---|---|
| **P0** | **清账** | 提交已完成但没提交的东西 | 你确认 §7-A 文案 | 小 |
| **P1** | **脱离终端** | `.app` 启动器 + 单实例锁 + PATH 补丁 | 无 | **小** |
| **P2** | **界面地基** | 抽 `panel.py`（**纯搬家**） | 无 | 中 |
...
| **P7** | **给朋友** | `install.sh` | P1 | 小 |

### 2.1 P0 具体是什么

就是**把已经做完但还挂在工作区里的东西先提交掉**，否则越堆越乱。
当前 7 个未提交项，分两批：

| 批次 | 文件 | 状态 |
|---|---|---|
| **批一** | `README.md`、`obsidian_writer.py`、`materials.example/` | **做完两轮了，只等你确认文案** |
| **批二** | `docs/RESEARCH-*.md` ×2、`docs/PLAN-roadmap.md`、`CLAUDE.md` | 调研产物 + 指针修复，随时可提 |

### 2.2 为什么是这个顺序

1. **P0 最先** —— 工作区已经堆了 7 项，里面还有"待确认"的，再拖会跟 P4/P5 的改动混在一起
2. **P1 排第二，因为它几乎不用写代码** —— 地基已经在了：`cl-bg.py` 的第一行 docstring
   就是「**把 ClassLive 从终端彻底剥离**——双重 fork + setsid」，而且它**故意复用 `cl` 那一层**
   （自己的注释：「不做第二份拷贝 —— 否则"两处各记一次"迟早漂移」）。缺的只是一个 `.app` 外壳
